...
output_file = 'Result/VGP_preds_MSE_NInd.csv'


if not os.path.exists(output_file):
    with open(output_file, 'w') as f:
        f.write('Iteration,MSE_VGP,MSE_estimated_VGP,func_loss\n')
...
for i in range(50):
    ...
    full_test_estimated_params_VGP = [item[0] for item in results]
    full_test_estimated_params_VGP_loss = [item[1] for item in results]

    full_test_estimated_params_VGP = np.array(full_test_estimated_params_VGP)
    ...
    Mean_loss = np.mean(full_test_estimated_params_VGP_loss)

    with open(output_file, 'a') as f:
        f.write(f"{i + 1},{MSE_VGP},{MSE_estimated_VGP},{Mean_loss}\n")



# nohup python VGP_NInd.py > VGP_NIndout.log 2>&1 &