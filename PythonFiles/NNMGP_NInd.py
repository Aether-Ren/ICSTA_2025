...
output_file = 'Result/NNMGP_preds_MSE_NInd.csv'


if not os.path.exists(output_file):
    with open(output_file, 'w') as f:
        f.write('Iteration,MSE_NNMGP,MSE_estimated_NNMGP,func_loss\n')
...
for i in range(50):
    ...
    full_test_estimated_params_NNMGP_loss = [item[2] for item in results]
    ...
    Mean_loss = np.mean(full_test_estimated_params_NNMGP_loss)

    with open(output_file, 'a') as f:
        f.write(f"{i + 1},{MSE_NNMGP},{MSE_estimated_NNMGP},{Mean_loss}\n")



# nohup python NNMGP_NInd.py > NNMGP_NIndout.log 2>&1 &